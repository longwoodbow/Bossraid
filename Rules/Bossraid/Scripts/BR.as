...
void Config(TDMCore@ this)//change here as config
{
	CRules@ rules = getRules();

	//how long to wait for everyone to spawn in?
	s32 warmUpTimeSeconds = 1;
	...
	//now it works as stock(todo)
	this.kills_to_win_per_player = -1;
	this.sudden_death = this.kills_to_win_per_player <= 0;

	//how long for the game to play out?
	f32 gameDurationMinutes = -1;
	...
	rules.set_f32("fall vel modifier", 1.2f);
}
...
shared class TDMCore : RulesCore
{
	...
	{
		TDMPlayerInfo p(player.getUsername(), 0, player.isBot() ? "knight" : (XORRandom(512) >= 256 ? "knight" : "archer"));//changed to 0 like sandbox
		players.push_back(p);
	...
	void CheckTeamWon()
	{
		...
		if (sudden_death)
		{
			...
			if (!team_alive)
			{
				...
				rules.SetGlobalMessage("We lost!");

				rules.set_s8("team_wins_on_end", 1);
				return;
			}

		}

		else warn("Sorry, non sudden death is not supported");

		// check boss health
		bool boss_defeat = true;

		CBlob@[] bosses;
		CMap@ map = getMap();
		if (getBlobsByTag("boss", @bosses))
		{
			for (uint i = 0; i < bosses.length; i++)// find not dead boss
			{
				CBlob @b = bosses[i];
		...
		if (boss_defeat)
		{
			...
			rules.SetCurrentState(GAME_OVER);
			rules.SetGlobalMessage("We won!");

			rules.set_s8("team_wins_on_end", 0);
			return;
		}
